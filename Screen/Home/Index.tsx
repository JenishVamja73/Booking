...
const Home = ({ navigation,route }) => {
  const [selectedSlots, setSelectedSlots] = useState([]); // Updated state for multiple selections
  ...
  const handleSlotSelection = (slotId, slotTime) => {
    ...
    // Toggle selection of the slot (add or remove)
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.bookingItem} onPress={() => EditUpdate(item)}>
  ...
  const EditUpdate = (item) => {
    // Check if item is valid and contains the required properties
    if (item && item.name && item.phone && item.time) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Select Show Times</Text>
        ...
        {slots.map((slot) => (
          <TouchableOpacity
            ...
            style={[
              ...
              selectedSlots.includes(slot.id) && styles.selectedSlot, // Highlight selected slots
            ]}
          ...
          >
            ...
            >
              {savedBooking.some((booking) => booking.time === slot.time)
                ? "Booked"
                : slot.time}
            </Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  selectedSlot: {
    backgroundColor: "#4caf50", // Highlight selected slots
  },
  slotText: {
    fontSize: 16,
    color: "#333",
